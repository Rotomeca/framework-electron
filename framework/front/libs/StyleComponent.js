...
class StyleComponent {
  ...
  get text() {
    ...
    if (keys.length) {
      for (
        let i = 0, len = keys.length, key = null, rule = null;
        i < len;
        ++i
      ) {
        key = keys[i];
  ...
  addCss(selector, { css_properties = {} } = {}) {
    return this.addRule(
      StyleComponent.CreateRule(selector, { css_properties }),
    );
  }
   ...
   * @return {StyleComponent} Chaînage
   */
  static Create(rules) {
    let component = new StyleComponent();
    component.addSomeCss(Rule.Write(rules));
    return component;
  }
...
class Rule {
  ...
  addProps(config) {
    if (config.addProps) {
      ...
      if (keys.length) {
        for (
          let i = 0, len = keys.length, key = null, value = null;
          i < len;
          ++i
        ) {
          key = keys[i];
  ...
  *[Symbol.iterator]() {
    ...
    for (let i = 0, len = keys.length; i < len; ++i) {
      if (this.#props[keys[i]])
        yield { property: keys[i], value: this.#props[keys[i]] };
    }
  }
